...
impl Plugin for DataBaseRequestHandlerPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(
            Update,
            (
                (listen_for_save_requests, save_to_data_base_and_system).chain(),
...
fn listen_for_save_requests(
    ...
    mut allow_player_to_set_board_name_event_writer: EventWriter<
        SetNewbornDomainBoardNameToDefault,
    >,
    ...
    for _save_request in event_reader.read() {
        if db_manager.get_saved_layouts_of_all_difficulties_count()
            >= super::MAX_SAVED_LAYOUTS as usize
        {
            save_outcome_event_writer.send(LayoutSaveAttemptOutcomeEvent(
                SaveAttemptOutcome::DataBaseAtCapacity,
            ));
        } else if let Some(existing_board_name) = DataBaseManager::domain_board_already_exists(
            &domain_boards_query,
...
fn save_to_data_base_and_system_inner(
    ...
    let layout_content_string =
        ron::ser::to_string_pretty(&save_request.board, ron::ser::PrettyConfig::default()).unwrap();
    write_to_file(
...
fn remove_from_data_base_and_system_inner(
    ...
    match db_manager.remove_layout_by_index_and_despawn_entity(
        layout_index,
        domain_board_name_query,
        commands,
    ) {
...
fn clear_db(db_manager: &mut DataBaseManager) -> Result<(), DataBaseError> {
    ...
    for valid_text_file_name in valid_text_file_names {
        let valid_text_file_name_excluding_postfix = valid_text_file_name.to_name();
