...
impl Plugin for NewbornDomainBoardNamePlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<NewbornDomainBoardName>().add_systems(
            Update,
            (
...
fn show_suggested_newborn_board_name(
    ...
    for _event in event_reader.read() {
        ...
        newborn_board_name_update_event_writer
            .send(UpdateNewbornDomainBoardName(suggested_board_name));
    }
}
...
fn determine_suggested_board_name(
    ...
    if *game_board_name != DomainBoardName::default() {
        newborn_domain_board_name.existing_boards_with_same_name_and_difficulty = db_manager
            .get_existing_boards_with_same_name_and_difficulty(game_board_name, board_name_query);
