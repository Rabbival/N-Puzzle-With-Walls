...
fn listen_for_set_confirm_allowed_requests(
    ...
    for request in event_reader.read() {
        if let Err(entity_error) = listen_for_set_confirm_allowed_requests_inner(
            request,
...
fn listen_for_newborn_domain_board_change_requests(
    ...
    mut text_above_pop_up_buttons_query: Query<
        &mut Text,
        (
            With<TextAbovePopUpMessageButtons>,
            Without<PopUpMessageDynamicTextTag>,
        ),
    >,
    pop_up_dynamic_text_query: Query<
        (&BackgroundColor, &Text),
        (
            With<PopUpMessageDynamicTextTag>,
            Without<TextAbovePopUpMessageButtons>,
        ),
    >,
) {
        ...
        match key_typed.keycode {
            KeyCode::Backspace | KeyCode::Delete => {
                if !pop_up_dynamic_text.is_empty() {
                    shorten_name(
                        &mut update_name_event_writer,
                        pop_up_dynamic_text,
                        first_set_since_default,
                    );
                }
            }
            keycode => {
                if let Some(parsed_keycode) =
                    try_get_string_from_keycode(keycode, key_typed.shift_pressed)
                {
                    if pop_up_dynamic_text.len() < MAX_DOMAIN_BOARD_NAME_LENGTH {
                        if first_set_since_default {
                            ...
                            update_name_event_writer.send(UpdateNewbornDomainBoardName(
                                DomainBoardName(format!(
                                    "{}{}",
                                    pop_up_dynamic_text, parsed_keycode
                                )),
                            ));
                        }
                    } else {
...
fn shorten_name(
    ...
    update_name_event_writer.send(UpdateNewbornDomainBoardName(DomainBoardName(String::from(
        shortened_name,
    ))));
}
...
fn set_newborn_board_displayed_name_and_message(
    ...
    >,
    mut text_above_pop_up_buttons_entity_query: Query<
        &mut Text,
        (
            With<TextAbovePopUpMessageButtons>,
            Without<PopUpMessageDynamicTextTag>,
    ...
    for name_request in event_reader.read() {
        ...
        let pop_up_dynamic_text = &mut pop_up_dynamic_text_query.single_mut().sections[0];
        ...
        let (new_pop_up_button_text, confirm_allowed) = if requested_name.0.is_empty() {
            (TextAbovePopUpButtonsType::MustGiveAName, false)
            ...
            .is_some()
        {
            ...
            (TextAbovePopUpButtonsType::NoText, true)
        };

        set_text_section_value_and_color(pop_up_dynamic_text, None, Some(requested_name.0.clone()));
        set_text_section_value_and_color(
...
fn listen_for_show_pop_up_to_set_newborn_board_name_requests(
    ...
    mut text_above_pop_up_buttons_query: Query<
        ...
        (
            With<TextAbovePopUpMessageButtons>,
            Without<PopUpMessageTextTag>,
        ),
    >,
    mut pop_up_text_query: Query<
        &mut Text,
        (
            With<PopUpMessageTextTag>,
            Without<TextAbovePopUpMessageButtons>,
        ),
    >,
) {
...
fn set_pop_up_message_text_and_type(
    ...
    let pop_up_text_ref = &mut pop_up_text_query.single_mut().sections[0].value;
    let (pop_up_message_entity, mut pop_up_message_type) = pop_up_message_query.single_mut();
    visibility_toggle_event_writer.send(SetEntityVisibility {
...
fn listen_for_delete_related_button_events(
    ...
    for action_request in event_reader.read() {
        let (pop_up_message_entity, pop_up_message_type) = pop_up_message_query.single();
...
fn listen_for_db_related_button_events(
    ...
    for action_request in event_reader.read() {
        if *pop_up_message_query.single() == PopUpMessageType::ChooseNewbornDomainBoardName {
            if let PopUpMessageButtonAction::Confirm = action_request.action {
                if let Some(newborn_domain_board_name) =
                    &newborn_domain_board_name_res.optional_name
                {
...
fn try_get_confirm_button_entity(
    buttons_query: &Query<(Entity, &PopUpMessageButtonAction)>,
) -> Option<Entity> {
