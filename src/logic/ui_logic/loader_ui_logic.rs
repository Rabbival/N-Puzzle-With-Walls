    ...
    fn build(&self, app: &mut App) {
        ...
        .add_systems(
            ...
            (
                ...
                (
                    update_slots_info_after_change.run_if(
                        resource_changed::<DataBaseManager>
                            .or_else(resource_changed::<DisplayedLoaderScreenNumber>),
                    ),
                    update_arrows_after_change.run_if(
                        resource_changed::<DataBaseManager>
                            .or_else(resource_changed::<DisplayedLoaderScreenNumber>),
                    ),
                )
                    .in_set(InputSystemSets::MainChanges),
                update_chosen_mark_after_change.run_if(
                    resource_changed::<ChosenLayoutLocation>
                        .or_else(resource_changed::<DisplayedLoaderScreenNumber>),
                ),
            ),
        );
        ...
        ) = event.action
        {
            ...
            *applied_props_difficulty = chosen_board_difficulty;
            ...
            event_writer.send(SetAppliedTagForProperty {
                give_tag_to_variant: MenuButtonAction::ChangeBoardDifficulty(
                    chosen_board_difficulty,
                ),
            });
        }
...
fn mark_chosen_slot_if_visible(
    ...
    for (entity, action, mut slot_background_color) in &mut loader_screen_actions_query {
        if let LoaderScreenAction::ChooseLayoutInSlot(layout_slot) = *action {
            if let Some(chosen_layout_location) = optional_chosen_layout_location.0 {
                ...
                if chosen_layout_location.screen_and_slot.screen == displayed_loader_screen_number.0
            ...
            commands.entity(entity).remove::<SelectedOptionTag>();
        }
    }
}
...
fn show_currently_displayed_saved_layouts_screen_inner(
    ...
    for currently_checked_screen_slot in all::<LoaderScreenSlot>() {
        ...
        let index_from_slot = SavedLayoutIndexInDifficultyVec::from_screen_and_slot(
        ...
        let optional_layout_to_display = data_base_manager.try_get_layout_ref(&index_from_slot);
...
fn hide_loader_slot(
    ...
    for (loader_action, mut layout_slot_on_screen_tag) in loader_screen_actions_query {
...
fn only_show_arrows_if_theres_more_than_one_available_screen(
    ...
    let applied_board_properties = applied_board_properties_query.single();
    let optional_current_dif_layouts_count = data_base_manager
        .get_layouts_count_by_difficulty(&applied_board_properties.board_difficulty);
