...
impl Plugin for ActiveLoaderSlotUpdaterPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(
            Update,
            (show_slot, set_slot_text).after(show_currently_displayed_saved_layouts_screen),
        );
    }
}
...
fn show_slot(
    ...
    for slot_set_request in event_reader.read() {
        for (loader_action, mut layout_slot_on_screen_tag) in &mut loader_screen_actions_query {
...
fn set_slot_text(
    ...
    for loader_slot_set_request in event_reader.read() {
        match domain_board_query.get(loader_slot_set_request.layout_entity) {
            ...
            Err(_query_entity_error) => {
                print_entity_related_error(EntityRelatedCustomError::EntityNotInQuery)
            }
        };
    }
}
        ...
        if let LoaderScreenAction::ChooseLayoutInSlot(layout_slot) = *loader_action {
            if layout_slot == slot_to_set {
                for child_entity in children.iter() {
                    let layout_slot_text_result = layout_slot_text_query.get_mut(*child_entity);
