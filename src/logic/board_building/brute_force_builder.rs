...
pub fn brute_force_generate_game_board(
    ...
    let mut location_shift_trackers: Vec<LocationShiftTracker> = empty_tile_locations
        .iter()
        .enumerate()
    ...
    for _shift in 0..location_shift_count {
        for shift_tracker in &mut location_shift_trackers {
            ...
            let valid_directions: Vec<&BasicDirection> =
                optional_directions.keys().clone().collect();
            ...
            if let Err(error) = board.swap_tiles_by_location(&empty_tile_location, chosen_location)
            {
                ...
                shift_tracker
                    .shift_direction_sequence
                    .push(*chosen_direction);
    ...
    for shift_tracker in location_shift_trackers {
        let reveresed_shift_order = shift_tracker.shift_direction_sequence.iter().rev().copied();
        print_to_console::print_possible_solution(shift_tracker.empty_index, reveresed_shift_order);
    }
...
mod tests {
    ...
    fn several_attempts_at_generating_unsolved_boards() {
        ...
        let solved_board = generate_solved_board(&BoardProperties::default()).unwrap();
