...
-> Result<Vec<GridLocation>, error_handler::BoardGenerationError>
{
    ...
    let mut possible_spawn_locations = neighbor_count_grid.all_initialized_locations_as_vec();
...
fn initialize_neighbor_count_grid(grid_side_length: u8) -> Grid<u8>{
    ...
    for edge_tile_not_corner in neighbor_count_grid.edges_without_corners_locations(){
        ...
        if MIN_NEIGHBORS == 3 {
            for neighbor_to_forbid in 
                neighbor_count_grid.get_all_direct_neighbor_locations(&edge_tile_not_corner)
            {
                neighbor_count_grid.set_none_get_former(&neighbor_to_forbid.1);
            }
        }
    }
    for corner in neighbor_count_grid.corner_locations(){
        ...
        if MIN_NEIGHBORS >= 2 {
            for neighbor_to_forbid in 
                neighbor_count_grid.get_all_direct_neighbor_locations(&corner)
            {
                neighbor_count_grid.set_none_get_former(&neighbor_to_forbid.1);
            }
        }
